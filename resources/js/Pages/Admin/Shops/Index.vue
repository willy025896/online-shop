...
<template>
    <AdminLayout title="Shops">
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Shops</h2>
        </template>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="shop in shops.data" :key="shop.id">
                        ...
                        <td class="px-6 py-4 text-right text-sm space-x-2">
                            <button v-if="shop.status !== 'approved'" @click="updateStatus(shop, 'approved')" class="text-green-600 hover:text-green-900">Approve</button>
                            <button v-if="shop.status !== 'suspended'" @click="updateStatus(shop, 'suspended')" class="text-red-600 hover:text-red-900">Suspend</button>
                        </td>
                    </tr>
                </tbody>
            </table>
