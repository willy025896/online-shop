        ...
        <div v-if="showForm" class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex gap-2">
                    <PrimaryButton :disabled="form.processing">{{ editing ? 'Update' : 'Create' }}</PrimaryButton>
                    <button type="button" @click="showForm = false" class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md">Cancel</button>
                </div>
            </form>
        </div>
            ...
            <table v-if="categories.length" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Subcategories</th>
                    ...
                    <template v-for="category in categories" :key="category.id">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right text-sm space-x-2">
                                <button @click="startEdit(category)" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                        ...
                        <tr v-for="child in category.children" :key="child.id" class="bg-gray-50 dark:bg-gray-750">
                            ...
                            <td class="px-6 py-3 text-right text-sm space-x-2">
                                <button @click="startEdit(child)" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                <button @click="deleteCategory(child)" class="text-red-600 hover:text-red-900">Delete</button>
                            </td>
                        </tr>
                    </template>
                </tbody>
