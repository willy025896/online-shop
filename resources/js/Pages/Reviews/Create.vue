...
<template>
    <AppLayout :title="lang.write_review || '撰寫評論'">
        <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">{{ lang.write_review || '撰寫評論' }}</h1>
                <p class="text-sm text-gray-500 mt-1">訂單 {{ order.order_number }}</p>
            </div>

            <!-- Cooling period notice -->
            <div v-if="coolingUntil" class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
            ...
            <div v-if="reviewableItems.length === 0" class="bg-white border border-gray-200 rounded-lg p-6 text-center text-gray-500">
                {{ lang.already_reviewed || '所有商品均已評論。' }}
            </div>

            <div v-else class="space-y-6">
                <div v-for="entry in forms" :key="entry.item.id" class="bg-white border border-gray-200 rounded-lg p-5">
                    <div v-if="entry.submitted" class="text-center py-4 text-green-600 font-medium">
                        ✓ 評論已送出
                    </div>
                    <template v-else>
                        <!-- Product info -->
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <div class="font-medium text-gray-900 text-sm">{{ entry.item.product_name }}</div>
                                <div class="text-xs text-gray-400">{{ entry.item.quantity }} 件</div>
                            </div>
                        </div>

                        <!-- Rating -->
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">{{ lang.rating || '評分' }}</label>
                        ...
                        <!-- Comment -->
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">{{ lang.comment || '評論內容（選填）' }}</label>
                            <textarea
