    ...
    <SellerLayout :title="c.title">
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('seller.coupons.create')" class="inline-flex items-center px-4 py-2 bg-brand-500 text-white text-sm font-medium rounded-md hover:bg-brand-600">
                    {{ c.add }}
                </Link>
            </div>
        </template>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-x-auto">
                    ...
                    <tr v-for="coupon in coupons.data" :key="coupon.id">
                        ...
                        <td class="px-6 py-4 text-right text-sm space-x-2">
                            <RowActions :loading="isDeleting(coupon.id)">
                                <Link :href="route('seller.coupons.edit', coupon.id)" class="text-brand-500 hover:text-brand-700">{{ c.action_edit }}</Link>
            ...
            <div v-else class="px-6 py-12 text-center text-gray-500">
                {{ c.no_coupons }} <Link :href="route('seller.coupons.create')" class="text-brand-500 hover:underline">{{ c.create_first }}</Link>.
            </div>
        </div>
