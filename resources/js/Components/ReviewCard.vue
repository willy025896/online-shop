    ...
    <div class="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
        ...
        <div class="flex items-start justify-between gap-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center font-semibold text-indigo-600 text-sm flex-shrink-0">
                    {{ review.user?.name?.charAt(0)?.toUpperCase() }}
                </div>
                <div>
                    <div class="font-medium text-gray-800 text-sm">{{ review.user?.name }}</div>
        ...
        <p v-else class="text-gray-400 text-sm italic">（無文字評論）</p>

        <!-- Seller reply -->
        <div v-if="review.seller_reply" class="bg-gray-50 border-l-4 border-indigo-300 rounded-r pl-3 pr-3 py-2">
            <div class="text-xs font-semibold text-indigo-600 mb-1">賣家回覆</div>
        ...
        <div v-if="showReplyForm && !review.seller_reply" class="pt-1">
            <form @submit.prevent="submitReply">
                ...
                <div class="flex justify-end gap-2 mt-2">
                    <button type="button" class="text-sm text-gray-500 hover:text-gray-700" @click="replyForm.reset()">取消</button>
                    <button
                        type="submit"
