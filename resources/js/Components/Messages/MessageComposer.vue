...
<template>
    <form @submit.prevent="submit" class="border-t border-gray-200 dark:border-gray-700 p-3 bg-white dark:bg-gray-800">
        ...
        <div class="flex items-end gap-2">
            <label class="cursor-pointer p-2 text-gray-500 hover:text-brand-500 transition">
            ...
            <textarea
                ...
                class="flex-1 resize-none rounded-2xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-sm focus:ring-accent-400 focus:border-brand-400"
            ></textarea>
            <button
                type="submit"
                :disabled="form.processing || (!form.body.trim() && !form.image)"
                aria-label="Send message"
                class="bg-brand-500 hover:bg-brand-600 disabled:bg-gray-400 text-white rounded-full p-2 transition"
            >
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
