...
date: 2026-07-09
status: Accepted（核心路徑已實作；已補 Pest 測試並跑過 post-change-review，審查中發現的問題已修正——見下方 Open Questions #4 與 #5）
---
...
4. （2026-07-10 post-change-review 後新增）`PaymentService::markAsPaid()` 原本在 `handleGatewayNotification()` 持有 `Order::lockForUpdate()` 的交易內同步呼叫發票開立 API——若 ECPay 發票端點變慢，會拉長這個 webhook 觸發的 row lock 持有時間。已透過拆分 `markAsPaid()`/`markAsPaidWithoutInvoice()`，讓 `handleGatewayNotification()` 在**交易 commit、鎖釋放之後**才呼叫 `InvoiceService::issueForOrder()`，解決最直接的風險，改動範圍小。**更徹底但這次先不做的做法**：比照本專案 9 個 `ShouldQueue` 通知類別的模式，把發票開立整個改成走 Laravel queue（`InvoiceService::issueForOrder()` dispatch 一個 job，而不是同步呼叫），讓外部 HTTP call 完全跟 request/webhook 的生命週期脫鉤——不只是搬到 transaction 之外，而是搬到當前 HTTP 請求之外。這樣即使 ECPay 發票 API 逾時或大幅變慢，也完全不影響 webhook 回應時間、不佔用任何 DB connection。目前的「交易外呼叫」已經解決了 row lock 的直接風險，先不做 queue 化；如果之後量大到「同步 HTTP 呼叫拖慢回應」本身成為問題，再評估升級成 queued job（並注意屆時 `issueForOrder()` 的冪等 guard 仍然成立，可以安全地被 queue worker 重試）。
5. （2026-07-10 post-change-review 後新增）`EcpayInvoiceGateway::allowance()`/`issue()` 原本讓 `AllowanceAmount`/`SalesAmount`（金額總額）跟 `Items[].ItemAmount`（逐項金額）各自獨立計算，在訂單有優惠券折扣或總額出現小數（例如百分比折扣券）時兩者對不起來，會被 ECPay 拒絕——已修正為在兩個方法內各自算出「品項總和」與「應對總額」的差額，超出時自動補一行「零頭調整」/「折讓折扣」品項沖平，確保 `sum(Items.ItemAmount)` 永遠等於送出的總額欄位。
