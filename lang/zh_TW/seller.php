<?php

return [
    'panel' => '賣家中心',
    'dashboard' => '儀表板',
    'total_products' => '商品總數',
    'active_products' => '上架商品',
    'pending_orders' => '待處理訂單',
    'total_revenue' => '總營收',
    'recent_orders' => '最近訂單',
    'view_all' => '查看全部',
    'no_orders' => '尚無訂單。',
    'shop_pending' => '您的店鋪正在審核中，部分功能可能受到限制。',

    'period_today' => '今天',
    'period_week' => '本週',
    'period_month' => '本月',
    'period_all' => '全部',

    'revenue' => '營收',
    'revenue_vs_prev' => '對比上期',
    'revenue_growth' => ':pct% 對比上期',
    'total_orders' => '訂單總數',
    'no_data' => '本期間無資料。',

    'order_status_dist' => '訂單狀態',
    'top_products' => '熱賣商品',
    'top_products_empty' => '尚無銷售資料。',
    'product_name' => '商品',
    'qty_sold' => '銷售量',
    'revenue_col' => '營收',
    'sales_trend' => '銷售趨勢',

    'low_stock' => '低庫存',
    'low_stock_empty' => '所有商品庫存充足。',
    'out_of_stock' => '已售罄',
    'stock_left' => '剩 :n 件',
    'low_stock_more' => '另有 :n 項低庫存商品',

    'widget_settings' => '自訂顯示',
    'widget_revenue' => '營收卡片',
    'widget_order_status' => '訂單狀態',
    'widget_top_products' => '熱賣商品',
    'widget_revenue_chart' => '銷售圖表',
    'widget_low_stock' => '低庫存',
    'save_preferences' => '儲存',

    'register' => [
        'title' => '註冊成為賣家',
        'subtitle' => '建立您的店鋪，開始販售。',
        'shop_name' => '店鋪名稱',
        'shop_url' => '店鋪網址',
        'description' => '店鋪描述',
        'submit' => '註冊店鋪',
    ],

    'products' => [
        'title' => '商品管理',
        'add' => '新增商品',
        'create' => '建立商品',
        'edit' => '編輯商品',
        'name' => '商品名稱',
        'category' => '分類',
        'no_category' => '無分類',
        'price' => '價格',
        'compare_price' => '原價',
        'stock' => '庫存',
        'status' => '狀態',
        'featured' => '推薦商品',
        'description' => '商品描述',
        'images' => '商品圖片',
        'actions' => '操作',
        'delete_confirm' => '確定要刪除「:name」嗎？',
        'no_products' => '尚無商品。',
        'create_first' => '建立您的第一個商品',
        'low_stock_filter' => '低庫存',
        'draft' => '草稿',
        'active' => '上架',
        'inactive' => '下架',
        'update' => '更新商品',
        'action_edit' => '編輯',
        'action_delete' => '刪除',
        'cancel' => '取消',
        'confirm' => '確認',
        'export' => '匯出 CSV',
        'import' => '匯入 CSV',
        'import_title' => '匯入商品',
        'import_file' => 'CSV 檔案',
        'import_submit' => '匯入',
        'import_result' => '匯入結果',
        'created' => '新增',
        'updated' => '更新',
        'failed' => '失敗',
        'row' => '列',
        'reason' => '原因',

        'variants' => [
            'title' => '規格與變體',
            'hint' => '若商品有尺寸、顏色等不同規格，可在此分別設定各規格的價格與庫存。未設定變體時，商品沿用上方的價格/庫存。',
            'add_option' => '新增規格',
            'option_name' => '規格名稱（例如：尺寸）',
            'remove_option' => '刪除規格',
            'add_value' => '新增值',
            'value_placeholder' => '值（例如：M）',
            'generate' => '自動產生所有組合',
            'no_variants' => '尚未設定任何變體。',
            'sku' => 'SKU',
            'price' => '價格',
            'compare_price' => '原價',
            'stock' => '庫存',
            'remove_variant' => '刪除',
            'save' => '儲存規格',
            'combination_exists' => '此規格組合已存在',
            'select_options_first' => '請先新增規格與值',
        ],
    ],

    'coupons' => [
        'title' => '折扣碼',
        'add' => '新增折扣碼',
        'create' => '建立折扣碼',
        'edit' => '編輯折扣碼',
        'update' => '更新折扣碼',
        'code' => '折扣碼',
        'type' => '類型',
        'type_percentage' => '百分比（%）',
        'type_fixed' => '固定金額',
        'value' => '折扣值',
        'value_hint_percentage' => '折扣百分比（0–100）。',
        'value_hint_fixed' => '折抵賣場小計的金額。',
        'min_spend' => '最低消費',
        'max_discount' => '折抵上限（僅百分比）',
        'usage_limit' => '總使用上限',
        'per_user_limit' => '每人使用上限',
        'unlimited' => '無限制',
        'starts_at' => '開始時間',
        'expires_at' => '結束時間',
        'is_active' => '啟用',
        'status' => '狀態',
        'active' => '啟用中',
        'inactive' => '已停用',
        'used' => '已使用',
        'discount' => '折扣',
        'actions' => '操作',
        'action_edit' => '編輯',
        'action_delete' => '刪除',
        'delete_confirm' => '確定要刪除折扣碼「:code」嗎？',
        'no_coupons' => '尚無折扣碼。',
        'create_first' => '建立您的第一個折扣碼',
        'cancel' => '取消',
        'confirm' => '確認',
    ],

    'orders' => [
        'title' => '訂單管理',
        'order' => '訂單',
        'customer' => '顧客',
        'items' => '商品',
        'total' => '合計',
        'status' => '狀態',
        'date' => '日期',
        'details' => '訂單詳情',
        'customer_info' => '顧客資訊',
        'name' => '姓名',
        'email' => '電子信箱',
        'shipping_name' => '收件人',
        'phone' => '電話',
        'address' => '地址',
        'shipping_carrier' => '物流公司',
        'tracking_number' => '追蹤單號',
        'carriers' => [
            'black_cat' => '黑貓宅急便',
            'hct' => '新竹物流',
            'post_office' => '中華郵政',
            'seven_eleven' => '7-ELEVEN（超商取貨）',
            'family_mart' => '全家（超商取貨）',
            'other' => '其他',
        ],
        'select_carrier' => '請選擇物流公司',
        'edit_shipment' => '編輯',
        'no_shipment_info' => '尚未填寫',
        'order_items' => '訂單商品',
        'product' => '商品',
        'price' => '價格',
        'qty' => '數量',
        'subtotal' => '小計',
        'shipping' => '運費',
        'discount' => '折扣',
        'update_status' => '更新狀態',
        'mark_as' => '標記為:status',
        'notes' => '顧客備註',
        'no_orders' => '尚無訂單。',
        'items_count' => ':count 件商品',
        'cancel_order' => '取消訂單',
        'cancel_reason' => '取消理由',
        'cancel_reason_prompt' => '請填寫取消此訂單的原因。',
        'reject_reason_prompt' => '請填寫拒絕此取消申請的原因。',
        'cancellation_requested' => '買家申請取消訂單',
        'approve_cancellation' => '同意取消',
        'reject_cancellation' => '拒絕',
        'cancelled_by_buyer' => '已依買家申請取消',
        'cancelled_by_seller' => '由您取消',
        'cancel' => '取消',
        'confirm' => '確認',
        'return_requested' => '買家申請退貨',
        'return_reason' => '退貨原因',
        'reject_return_prompt' => '請填寫拒絕此退貨申請的原因。',
        'approve_return' => '同意退貨',
        'reject_return' => '拒絕',
        'return_approved' => '已核准退貨，退款 $:amount',
        'return_rejected' => '已拒絕退貨申請',
        'refunded_amount' => '已退款',
        'statuses' => [
            'pending' => '待付款',
            'paid' => '已付款',
            'processing' => '處理中',
            'shipped' => '已出貨',
            'completed' => '已完成',
            'cancelled' => '已取消',
        ],
    ],

    'shop' => [
        'title' => '店鋪設定',
        'current_logo' => '目前 Logo',
        'logo' => '店鋪 Logo',
        'name' => '店鋪名稱',
        'description' => '描述',
        'save' => '儲存變更',
    ],

    'payouts' => [
        'title' => '撥款紀錄',
        'no_payouts' => '尚無撥款紀錄。',
        'paid_at' => '撥款時間',
        'orders' => '訂單數',
        'gross_amount' => '商品淨額',
        'commission_amount' => '平台抽成',
        'shipping_amount' => '運費',
        'net_amount' => '實付金額',
    ],
];
