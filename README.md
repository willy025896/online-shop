...
- [ ] **`onError` 缺漏導致靜默失敗** - `Orders/Show.vue`（付款／詢問賣家）、`Notifications/Index.vue`（已讀／刪除）、`CartItemRow.vue`（數量／刪除）、`Admin/Users`、`Admin/Shops`（角色／狀態變更）等非 `useForm` 的 `router.post/patch/delete` 呼叫未帶 `onError`，違反 CLAUDE.md 既有慣例
